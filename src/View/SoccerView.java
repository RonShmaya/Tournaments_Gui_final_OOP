...
public class SoccerView extends GameView {
	private List<TextField> firstPersTieScore;
	private List<TextField> secondPersTieScore;
	private int counterTie;

	public SoccerView(eChmpStatus roundChmp, int numOfGame, int gameStages, String nameFirstPers, String nameSecPers,
			MainView mainView) {
		super(roundChmp, numOfGame, gameStages, nameFirstPers, nameSecPers, Soccer.class.getSimpleName(), mainView);
			...
			public void handle(ActionEvent event) {
				try {
					...
					if (isTie) {
						...
						for (int i = 0; i < Soccer.LIMIT_IS_2_HALF; i++) {
					...
					if (msg.getText().equals(eMsgForUser.Tie.toString())) {
						if (counterTie != 0) {
							msg.setText("please Add Kicks Result-> until 5 or difference 1");
						}
						tieShow();
						isTie = true;
	...
	private void startShow() {
		for (int i = 0; i < Soccer.LIMIT_TIE_TIMES; i++) {
			...
			detailsOrder.add(firstPersTieScore.get(i), i + Soccer.LIMIT_IS_2_HALF + 1, 0);
			detailsOrder.add(secondPersTieScore.get(i), i + Soccer.LIMIT_IS_2_HALF + 1, 1);
		}
		firstPersTieScore.get(firstPersTieScore.size() - 1).setText("Kick");
		secondPersTieScore.get(secondPersTieScore.size() - 1).setText("Kick");
	}
